...
class GpReprojectAndCleanupAllFramesOperator(bpy.types.Operator):
    """Project to view for all frames"""
    bl_idname = "gpencil.reproject_cleanup_all_frames"
    ...
    def execute(self, context):
        ...
        print(len(animated_frames), sorted(animated_frames))
...
def menu_func(self, context):
    self.layout.operator(
        GpReprojectAndCleanupAllFramesOperator.bl_idname, text=GpReprojectAndCleanupAllFramesOperator.bl_label
    )


def register():
    bpy.utils.register_class(GpReprojectAndCleanupAllFramesOperator)
    bpy.types.VIEW3D_MT_edit_gpencil.append(menu_func)


def unregister():
    bpy.utils.unregister_class(GpReprojectAndCleanupAllFramesOperator)
